...
public class SimpleJdbcClinic implements Clinic, SimpleJdbcClinicMBean {
	...
	public void init(DataSource dataSource) {
		...
		this.insertOwner = new SimpleJdbcInsert(dataSource).withTableName(
				"owners").usingGeneratedKeyColumns("id");
		this.insertPet = new SimpleJdbcInsert(dataSource).withTableName("pets")
				.usingGeneratedKeyColumns("id");
		this.insertVisit = new SimpleJdbcInsert(dataSource).withTableName(
				"visits").usingGeneratedKeyColumns("id");
	}
		...
		synchronized (this.vets) {
			...
			for (Vet vet : this.vets) {
				...
				for (int specialtyId : vetSpecialtiesIds) {
					Specialty specialty = EntityUtils.getById(specialties,
							Specialty.class, specialtyId);
	...
	public Pet loadPet(int id) throws DataAccessException {
		JdbcPet pet;
		try {
			pet = this.namedParameterJdbcTemplate.getJdbcOperations()
					.queryForObject(
							"SELECT id, name, birth_date, type_id, owner_id FROM pets WHERE id=?",
							new JdbcPetRowMapper(), id);
		} catch (EmptyResultDataAccessException ex) {
			throw new ObjectRetrievalFailureException(Pet.class,
					new Integer(id));
		}
	...
	public void storeVisit(Visit visit) throws DataAccessException {
		if (visit.isNew()) {
			Number newKey = this.insertVisit
					.executeAndReturnKey(createVisitParameterSource(visit));
	...
	private void loadPetsAndVisits(final Owner owner) {
		final List<JdbcPet> pets = this.namedParameterJdbcTemplate.getJdbcOperations()
				.query("SELECT id, name, birth_date, type_id, owner_id FROM pets WHERE owner_id=?",
						new JdbcPetRowMapper(), owner.getId().intValue());
